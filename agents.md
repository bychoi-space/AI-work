...
- **MutationObserver 무한 재귀 루프 방지**: Textbox/Textarea와 같이 `contenteditable`이 포함된 컴포넌트는 `enforceDesignSystem()` 등에서 내부 DOM 속성(예: 카운터 `textContent`, 플레이스홀더 `display` 등)을 업데이트할 때 Mutation이 발생하여 무한 재귀 루프가 생길 수 있습니다. 따라서 모든 DOM 쓰기 작업은 반드시 **이전 값과 신규 값의 엄격한 비교 가드(Value Comparison Guard)**를 통과한 경우에만 실행되어야 합니다.
