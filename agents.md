...
- **양측 여백 및 테이블 크기 균형 가이드 (Balanced Layout Breathing Room)**:
  ...
  - 컨텐츠 양에 맞게 각 열의 가로폭을 최적의 픽셀(예: 날짜 열 `140px` ~ `160px`)로 차분히 줄이고, 테이블 전체 가로폭(`comp-main-table`의 `width` 값) 역시 컴팩트하게 축소하여 좌측 여백(`40px`)과 우측 여백이 조화롭고 대칭적인 여백미를 갖추도록 균형 있게 정비해야 합니다.
