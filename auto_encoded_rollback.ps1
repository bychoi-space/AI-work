...
# Define the RAW stable scripts (09:36 AM logic)
$rawUndo = @"
(function() { 
    var stack = []; 
...
(function() { 
    ...
    window.addEventListener("message", function(e) { 
        ...
        if (d.type === "LF_SNAP_RESPONSE" && activeEl && isDragging) { 
            ...
            v.innerHTML = d.html + '<div class="lf-drag-handle">::</div><div class="lf-resizer"></div><div class="lf-delete-trigger">×</div>'; 
            host.appendChild(v); 
...
# Encode to Base64
$b64Undo = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($rawUndo))
$b64Engine = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($rawEngine))

$coreCode = @"
/**
 * vctrl_core.js - Central Orchestrator for LF Editor Studio
 * Real-time Base64 Shielded Version (Stable 09:36 AM)
 */
window.MessageHub = {
    handlers: {},
...
window.loadScreen = async function(fileName) {
    ...
    var scriptTag = '<script id=\"v4-script\">' + 
        'eval(decodeURIComponent(escape(atob(\"$b64Undo\"))));' +
        'eval(decodeURIComponent(escape(atob(\"$b64Engine\"))));' +
        '</script>';
...
Write-Host "Auto-Encoded Base64 Rollback Successful."
