...
(function() {
    ...
    document.addEventListener('mousedown', e => {
        ...
        if (c) {
            ...
            // Check if already selected and clicking an editable cell
            const isEditableClick = e.target.closest('.v4-editable-cell');
            ...
            if (!isEditableClick || !isAlreadySelected) {
                notifyParent({ 
                    type: "LF_COMP_SELECTED", 
                    shiftKey: isMulti,
                    ...window._getCompStyles(c)
        ...
        if (r) { 
            ...
            // If clicking inside an editable cell (like a grid cell), do not start dragging
            if (e.target.closest('.v4-editable-cell')) {
                return;
            }
            if (window.V4UndoManager) window.V4UndoManager.saveState();
    ...
    document.addEventListener('input', e => { 
        if (e.target.classList.contains('v4-editable-cell')) {
            ...
            if (comp) {
                if (typeof window.enforceDesignSystem === 'function') {
                    window.enforceDesignSystem();
                } else if (typeof window.resizeAtomToFitText === 'function') {
    ...
    // Allow direct cell selection, click focusing, and editing inside Grid UI components
    document.addEventListener('mousedown', function(e) {
        var cell = e.target.closest('.v4-grid-cell.v4-editable-cell');
        if (cell) {
            // Stop selection and dragging handlers from intercepting the mousedown
            e.stopPropagation();
        }
    });
    ...
    window.renderGrid = function(container, columns, rowCount, showPagination) {
        ...
        // Calculate explicit table width based on columns
        var totalWidth = 0;
        var hasFlexCol = false;
        columns.forEach(function(col) {
            var w = col.width || '100px';
            w = w.trim();
            if (w.includes('fr') || w.includes('%') || w === 'auto') {
                hasFlexCol = true;
            } else {
                var val = parseFloat(w) || 100;
                totalWidth += val;
            }
        });
        var tableWidthStyle = hasFlexCol ? 'width: 100%;' : 'width: ' + totalWidth + 'px;';
        ...
        var getStatusBadgeHtml = function(col, rowIndex) {
            var optsStr = col.options || '방송중, 방송예정, 방송종료';
            var opts = optsStr.split(',').map(function(s) { return s.trim(); }).filter(Boolean);
            if (opts.length === 0) opts = ['상태'];
            var statusText = opts[rowIndex % opts.length];
            
            var badgeStyles = [
                { bg: 'rgba(52,211,153,0.15)', color: '#10b981' }, // Emerald / Green
                { bg: 'rgba(251,191,36,0.15)', color: '#d97706' }, // Amber / Yellow
                { bg: 'rgba(239,68,68,0.1)', color: '#ef4444' },    // Red
                { bg: 'rgba(59,130,246,0.15)', color: '#3b82f6' },  // Blue
                { bg: 'rgba(139,92,246,0.15)', color: '#8b5cf6' },  // Purple
                { bg: 'rgba(244,63,94,0.15)', color: '#f43f5e' },   // Pink
                { bg: 'rgba(100,116,139,0.15)', color: '#64748b' }  // Grey
            ];
            
            var styleIdx = rowIndex % opts.length;
            var style = badgeStyles[styleIdx % badgeStyles.length];
            
            return '<span style="background:' + style.bg + '; color:' + style.color + '; padding:2px 6px; border-radius:4px; font-size:11px; font-weight:600;">' + statusText + '</span>';
        };
        ...
        if (table) {
            ...
            if (hasFlexCol) {
                ...
                table.style.minWidth = ''; // Clear min-width so the table can shrink below 100%
            }
                        ...
                        if (shouldOverwrite) {
                            ...
                            if (col.type === 'checkbox') {
                                ...
                                if (col.type === 'number') {
                                    ...
                                    td.innerHTML = getStatusBadgeHtml(col, rIdx);
        ...
        var headerHtml = '<tr style="height:36px; background:#f8fafc; border-bottom:1.6px solid rgb(226,232,240); box-sizing:border-box;">';
        ...
        for (var i = 0; i < rowCount; i++) {
            ...
            columns.forEach(function(col, colIndex) {
                ...
                if (col.type === 'checkbox') {
                    ...
                    bodyHtml += '<td class="v4-grid-cell v4-editable-cell" contenteditable="true" data-type="status" style="display:table-cell; vertical-align:middle; text-align:left; padding:0 8px;' + borderRight + ' box-sizing:border-box;">' + getStatusBadgeHtml(col, i) + '</td>';
                } else if (col.type === 'author') {
        ...
        var tableContainerHtml = '<div class="v4-grid-table-wrapper" style="width:100%; height:' + tableHeight + '; overflow:auto; box-sizing:border-box;">' +
                                 '<table style="' + tableWidthStyle + ' table-layout:fixed; border-collapse:collapse; background:#ffffff; box-sizing:border-box;">' +
                                 colgroupHtml +
    ...
    window.addEventListener('message', e => {
        ...
        if (d.type === 'LF_SNAP_RESPONSE' && activeEl && isDragging) {
            ...
            function syncTableComponentSize() {
                const s = document.querySelector('.lf-component.selected');
                if (!s) return;
            ...
            if (isGrid) {
                ...
                markDirty();
                setTimeout(syncTableComponentSize, 50);
                return;
            }
    ...
    // 7. Mobile UI 설계 - 1 Screen
    'template_mobile_ui_1.html': `<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    // 8. Mobile UI 설계 - 2 Screens
    'template_mobile_ui_2.html': `<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    // 9. Mobile UI 설계 - 3 Screens
    'template_mobile_ui_3.html': `<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    // 11. PC UI Template (Google Chrome Browser Frame)
    'template_pc_ui.html': `<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
            ...
            <div class="chrome-header">
                <div class="chrome-top-bar">
                    ...
                    <div class="chrome-tabs">
                        <div class="chrome-tab">
                            ...
                            <span style="font-family: inherit;">LFmall | Premium Design</span>
                ...
                <div class="chrome-nav-bar">
                    ...
                    <div class="chrome-url-bar">
                        ...
                        <span>https://www.lfmall.co.kr</span>
                    </div>
